...
-- H2 has no SQL-language table functions: the stored function is a Java-source alias that H2
-- compiles at runtime (H2 injects the leading Connection parameter). Because the Java body
-- contains semicolons, the Liquibase changelog loads this file with splitStatements="false".
CREATE ALIAS get_orders_by_product AS $$
import java.sql.*;
@CODE
ResultSet getOrdersByProduct(Connection connection, String series, String code) throws SQLException {
    PreparedStatement statement = connection.prepareStatement(
            "SELECT * FROM orders_reports r WHERE r.product_series = ? AND r.product_code = ? ORDER BY r.creation_date DESC");
